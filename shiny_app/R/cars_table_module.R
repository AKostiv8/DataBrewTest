  ...
  tagList(
    ...
    fluidRow(
      column(
        ...
        tags$br(),
        tags$br(),
        tags$div(class="materialBox",
          tags$h4("CHANGES"),
          DTOutput(ns('car_table_vc')) 
        )
      )
    ),
...
cars_table_module <- function(input, output, session, auth) {
    ...
    tryCatch({
      out <- conn %>%
        ...
        mutate(
          ...
          modified_at = as_datetime(modified_at)
        ) %>%
  ...
  # observeEvent(input$`auth-go_auth`, {
  #   print("")
  # })
  
  userDataPermission <- reactive({
    req(auth$user_info)
    auth$user_info
  })
  ...
  observe({
    # req(auth$result)
    # req(check_credentials)
    # print(check_credentials)
    # print(session$userData$perminssion)
    # print(reactiveValuesToList(session$userData$perminssion))
    req(cars())
    req(userDataPermission())
    
    # print(userDataPermission())
  ...
  car_table_prep_vc <- reactiveVal(NULL)
  
  
  output$car_table_vc <- renderDT({
    req(car_table_prep_vc())
    out <- car_table_prep_vc()
    # req(cars_vc())
    # out <- car_table_prep_vc()
    ...
    datatable(
      out %>%
        ...
        mutate(modified_at = as_datetime(modified_at)) %>%
        arrange(desc(modified_at)),
      rownames = FALSE,
      colnames = c('Status', 'Model', 'Miles/Gallon', 'Cylinders', 'Displacement (cu.in.)',
                   'Horsepower', 'Rear Axle Ratio', 'Weight (lbs)', '1/4 Mile Time',
                   'Engine', 'Transmission', 'Forward Gears', 'Carburetors', 'Created At',
                   'Created By', 'Modified At', 'Modified By'),
      selection = "none",
  ...
  observeEvent(cars_vc(), {
    ...
    # Set the Action Buttons row to the first column of the `mtcars` table
    car_table_proxy_vt <- DT::dataTableProxy('car_table_vc')
    
    if (is.null(car_table_prep_vc())) {
      # loading data into the table for the first time, so we render the entire table
      # rather than using a DT proxy
      car_table_prep_vc(out)
      
    } else {
      
      # table has already rendered, so use DT proxy to update the data in the
      # table without rerendering the entire table
      replaceData(car_table_proxy_vt, out, resetPaging = FALSE, rownames = FALSE)
